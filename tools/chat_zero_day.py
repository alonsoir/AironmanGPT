...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="Path to draft model directory",
)
parser.add_argument(
    "-nds",
    "--no_draft_scale",
...
parser.add_argument(
    ...
    type=float,
    default=0.0,
    help="Sampler top-A, default = 0.0 (0 to disable)",
)
parser.add_argument(
    "-skew",
    "--skew",
    type=float,
    default=0.0,
    help="Skew sampling, default = 0.0 (0 to disable)",
)
parser.add_argument(
    "-typical",
    "--typical",
...
parser.add_argument(
    "-c8", "--cache_8bit", action="store_true", help="Use 8-bit (FP8) cache"
)
...
parser.add_argument(
    ...
    "--ngram_decoding",
    action="store_true",
    help="Use n-gram speculative decoding",
)
...
parser.add_argument(
    ...
    "--print_timings",
    action="store_true",
    help="Output timings after each prompt",
)
parser.add_argument(
    "-amnesia",
    "--amnesia",
    action="store_true",
    help="Forget context after every response",
)
...
generator = ExLlamaV2StreamingGenerator(
    model, cache, tokenizer, draft_model, draft_cache
)
...
while True:
    ...
    if print_timings:
        ...
        if args.ngram_decoding or draft_model is not None:
        ...
        if response_tokens == max_response_tokens:
            ...
            if tokenizer.eos_token_id in generator.stop_tokens:
                responses_ids[-1] = torch.cat(
                    [responses_ids[-1], tokenizer.single_token(tokenizer.eos_token_id)],
                    dim=-1,
                )
